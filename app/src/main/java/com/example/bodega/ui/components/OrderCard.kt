...
fun OrderCard(
    ...
    Card(
        ...
        Column(modifier = Modifier.padding(16.dp)) {
            Text(text = "Pedido #${order.order.orderId}", style = MaterialTheme.typography.titleMedium)
            Text(text = "Cliente: ${order.customer.firstName} ${order.customer.lastName}", style = MaterialTheme.typography.bodyMedium)
            Text(
                text = "Fecha: ${SimpleDateFormat("dd/MM/yyyy", Locale.getDefault()).format(order.order.orderDate)}",
